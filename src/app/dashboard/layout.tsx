...
// Define page access control list
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    // If auth state is resolved and there's no user, redirect to home.
  ...
  // After loading, if there's still no user or claims, we'll be redirecting, so we can show a loader.
  if (!user || !claims) {
    return <FullScreenLoader />;
  }
  ...
  const matchingPath = Object.keys(pageAccess)
    ...
    .sort((a, b) => b.length - a.length)[0]; // Sort by length for specificity
